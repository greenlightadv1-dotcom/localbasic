...
import { saveWebsiteAction, type WebsiteState } from './actions';
...
const FIELD = 'w-full rounded border border-line bg-elevated px-3 text-sm text-fg';
    ...
    <form action={action} className="space-y-6">
      ...
      <label className="block">
        ...
        <span className="mb-2 block text-sm text-muted">
          رابط صورة يبدأ بـ https. اتركه فارغًا لاستخدام خلفية بألوان المطعم.
        </span>
        <input name="heroUrl" type="url" maxLength={500} dir="ltr"
          placeholder="https://…" defaultValue={settings.heroUrl}
          className={`${FIELD} h-12`} />
      </label>
              ...
              {!closed[i] ? (
                <>
                  <input type="time" name={`opens-${i}`}
                    ...
                    aria-label={`${day} من`}
                    className="h-10 rounded border border-line bg-elevated px-2 text-sm text-fg" />
